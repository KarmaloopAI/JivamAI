...
# Usage: curl -fsSL https://jivamai.com/install.sh | bash
#
# Mirrored from KarmaloopAI/Jivam's scripts/install.sh — keep the two in sync
# whenever the install flow changes (see that repo's CLAUDE.md).
set -euo pipefail
